...
export default function Menu() {
  ...
  return (
    <div className="card p-4 shadow-1">
      <Button onClick={() => setVisible(true)} icon="pi pi-bars" />

      <Sidebar
        visible={visible}
        onHide={() => setVisible(false)}
        content={<Content hide={() => setVisible(false)} />}
      ></Sidebar>
    </div>
  );
}
...
const Content = ({ hide }) => {
  ...
  return (
    <div className="min-h-screen flex relative static surface-ground">
      <div
        id="app-sidebar-2"
        className="surface-section h-screen hidden block flex-shrink-0 absolute static left-0 top-0 z-1 border-right-1 surface-border select-none"
        style={{ width: "280px" }}
      >
        <div className="flex flex-column h-full">
          <div className="flex align-items-center justify-content-around px-4 pt-3 flex-shrink-0">
                ...
                <ul className="list-none p-0 m-0 overflow-hidden">
                  <li className="mb-2">
                    <NavLink
                      to="/"
                      className={({ isActive }) =>
                        `link-no-underline p-ripple flex align-items-center cursor-pointer p-3 border-round text-700 transition-duration-150 transition-colors w-full ${
                          isActive ? "bg-primary" : "hover:surface-100"
                        }`
                      }
                      onClick={() => hide()}
                    >
                  ...
                  <li className="mb-2">
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `link-no-underline p-ripple flex align-items-center cursor-pointer p-3 border-round text-700 transition-duration-150 transition-colors w-full ${
                          isActive ? "surface-100" : "hover:surface-100"
                        }`
                      }
                      onClick={() => hide()}
                    >
